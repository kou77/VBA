...
Public Function is_numstr(ByVal s As String) As Boolean
    Dim i As Long, mi As Long: mi = Len(s)
    For i = 1 To mi
        If is_number(s, i) = False Then Exit Function
    Next
    is_numstr = True
End Function
...
Public Function read_text_file(path As String, line_ary() As String, Optional lf_code As String = vbCrLf) As Boolean
    On Error GoTo Err
    Dim buf As String
    If read_file(path, buf) = False Then
        Exit Function
    End If
    line_ary = Split(chomp(buf), lf_code)
    read_text_file = True
Err:
    If Err.number <> 0 Then
...
Public Function write_csv_file(ByVal path As String, data_ary() As String, _
        Optional ByVal separator As String = ",", Optional qmflg As Boolean = False) As Boolean
    On Error GoTo Err
    Dim i As Long, mi As Long, j As Long, mj As Long
    ...
    Else
        Dim qmc As String
        For i = 0 To mi
            get_qm_chr data_ary(i, 0), qmc
            line_ary(i) = qmc & data_ary(i, 0) & qmc
            For j = 1 To mj
                get_qm_chr data_ary(i, 0), qmc
                line_ary(i) = line_ary(i) & separator & qmc & data_ary(i, j) & qmc
            Next
        Next
    End If
    write_csv_file = write_text_file(path, line_ary)
Err:
    If Err.number <> 0 Then
        output_trace_log errlog, "write_csv_file(): " & Err.description
    End If
End Function

Public Function write_utf8_file(ByVal path As String, ByVal buf As String) As Boolean
    On Error GoTo Err
    ...
    st.Close
    Set st = Nothing
    write_utf8_file = True
Err:
    If Err.number <> 0 Then output_log errlog, "write_utf8_file(): " & Err.description
End Function
'
...
Private Sub tes_chomp()
'    MsgBox chomp("test" & vbCrLf) & "]]]"
    MsgBox chomp("test" & vbCrLf & "]]]")
'    MsgBox chomp("") & "]]]"
    MsgBox "aaa" & vbCr & "]]]"
    MsgBox chomp("aaa" & vbCr) & "]]]"
End Sub

Public Function chomp(s As String) As String
    If Right(s, Len(vbCrLf)) = vbCrLf Then
        chomp = Left(s, Len(s) - Len(vbCrLf))
    ElseIf Right(s, Len(vbCr)) = vbCr Then
        chomp = Left(s, Len(s) - Len(vbCr))
    ElseIf Right(s, Len(vbLf)) = vbLf Then
        chomp = Left(s, Len(s) - Len(vbLf))
    Else
        chomp = s
    End If
End Function

Public Function get_lngary_maxidx(a() As Long, ByVal d As Long) As Long
    On Error GoTo Err
    get_lngary_maxidx = -1
    get_lngary_maxidx = UBound(a, d)
Err:
End Function

Public Function get_strary_maxidx(a() As String, ByVal d As Long) As Long
    On Error GoTo Err
    get_strary_maxidx = -1
    get_strary_maxidx = UBound(a, d)
Err:
End Function
